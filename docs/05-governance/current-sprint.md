...
- 2026-08-21: Closed Sprint 5. PR #14 (`feat/sprint5-gateway-registry-baseline`) merged to `main` with all required checks passing (run `32520564986`, 16/16 green). Closeout included: dependency security refresh completed (pnpm-honored overrides for `js-yaml`/`immutable`, stale `@angular/core@21.2.9` lockfile resolutions deduped, phantom `electron-updater` import declared, `pnpm audit --prod` reports no known vulnerabilities), repo-wide prettier drift resolved, and unused Azure Static Web Apps workflow removed to eliminate a permanent false-failing check.
